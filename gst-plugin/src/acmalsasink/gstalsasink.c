...
no_channels:
  {
    ...
    if ((alsa->channels) == 1)
      msg = g_strdup ("Could not open device for playback in mono mode.");
    if ((alsa->channels) == 2)
      msg = g_strdup ("Could not open device for playback in stereo mode.");
    if ((alsa->channels) > 2)
      msg =
          g_strdup_printf (
...
device_disappeared:
  {
    GST_ELEMENT_ERROR (asink, RESOURCE, WRITE,
        ("Error outputting to audio device. "
