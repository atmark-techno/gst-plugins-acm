...
/**
 * struct fb_dmabuf_export - export frame buffer as DMABUF file descriptor
 *
 * @index:	id number of the buffer (0 to number of planes)
 * @fd:		file descriptor associated with DMABUF
 * @flags:	flags for newly created file, currently only O_CLOEXEC is supported.
 */
...
struct fb_ops {
	...
	struct dma_buf *(*fb_dmabuf_get)(struct fb_info *info, unsigned int index);
#endif
};
